...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    val t = s.flatten.combinations(2).filter(x => x.head._1 != x.last._1).toList
    List()::s:::t
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft(Map(): Map[Char, Int])((r, p) => {
      val containValue = y.filter(x => x._1 == p._1)
      containValue match {
        case Nil => r + p
        case z::Nil => r+(z._1 -> (p._2 - z._2))
      }
